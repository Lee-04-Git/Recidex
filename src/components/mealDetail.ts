...
export function createMealDetail(meal: Meal): HTMLElement {
  ...
  detailContainer.innerHTML = `
    ...
    meal.strMeal
      ...
      <button class="favourite-detail-btn ${
        isFavourite ? "active" : ""
  ...
  const favouriteBtn = detailContainer.querySelector(
...
function toggleDetailFavourite(
  mealId: string,
...
function extractIngredients(meal: Meal): string[] {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (ingredient && ingredient.trim()) {
      ...
      ingredients.push(
        measureText ? `${measureText} ${ingredient}` : ingredient
      );
    }
  }
