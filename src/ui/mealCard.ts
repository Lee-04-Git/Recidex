...
export function createMealCard(meal: Meal): HTMLElement {
  ...
  card.innerHTML = `
    <img src="${meal.strMealThumb}" alt="${meal.strMeal}" class="meal-card-image" loading="lazy">
    <div class="meal-card-content">
      <div class="meal-card-header">
        <h3 class="meal-card-title">${meal.strMeal}</h3>
        <button class="favourite-btn ${isFavourite ? 'active' : ''}" data-meal-id="${meal.idMeal}">
  ...
  card.addEventListener('click', (e) => {
    ...
    if (!(e.target as HTMLElement).classList.contains('favourite-btn') && 
