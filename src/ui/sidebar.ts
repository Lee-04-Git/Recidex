...
export async function initSidebar(): Promise<void> {
  try {
    ...
    renderCategories();
    // Auto-select Beef category on initial load
    const beefCategory = categories.find(cat => cat.strCategory === 'Beef');
    if (beefCategory) {
      const beefCategoryElement = document.querySelector('.category-item[data-category="Beef"]') as HTMLElement;
      if (beefCategoryElement) {
        beefCategoryElement.classList.add('active');
      }
    }
  } catch (error) {
